...
const presidentsBetween1850and1900 = presidents.filter((president)=>{
  const splitTookOfficeDate = president.took_office.split("-")
  const tookOfficeDate = parseInt(splitTookOfficeDate[0])
  return tookOfficeDate >= 1850 && tookOfficeDate <= 1900
})
...
const shortTermPresidents = presidents.filter((president)=>{
  ...
  const durationOfTerm = leftOffice - tookOffice
  return durationOfTerm < 4
})
